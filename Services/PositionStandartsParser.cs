...
namespace Diplom.Services
{
    ...
    {
        public List<LoadedFileDto> GetFiles()
        {
        ...
        public PositionStandart ParseKind2Document(string fileName, MemoryStream ms)
        {
            PositionStandart standart = new PositionStandart();
            ...
            using (WordprocessingDocument doc = WordprocessingDocument.Open(ms, false))
            {
                ...
                if (table != null)
                {
                    ...
                    {
                        standart.Name = GetCellText(rows, passportIndex + 1);
                        standart.StandartDescription = GetCellText(rows, passportIndex + 4);
                    }

                    // Extracting General Info
                    int generalInfoIndex = rows.FindIndex(r => r.InnerText.Contains("Профессиональный стандарт"));
                    if (generalInfoIndex != -1)
                    {
                        standart.GeneralInfo = GetCellText(rows, generalInfoIndex + 1);
                    }
                    ...
                    for (int i = 0; i < rows.Count; i++)
                    {
                        try
                        {
                            if (rows[i].InnerText.Contains("КАРТОЧКА ПРОФЕССИИ"))
                            {
                                ...
                                // Relative indices for the card details
                                card.Code = GetCellText(rows, i + 1, 1);
                                card.Name = GetCellText(rows, i + 3, 1);
                                var orkLevelText = GetCellText(rows, i + 5, 1);
                                if (orkLevelText is not null
                                    && orkLevelText != "-"
                                    && int.TryParse(orkLevelText[0].ToString(), out int level)
                                    )
                                ...
                                for (int j = i + 3; j < rows.Count; j++)
                                {
                                    try
                                    {
                                        if (rows[j].InnerText.Contains("Трудовая функция"))
                                        {
                                            ...
                                            //function.Skills = ExtractListItems(rows, j).Where(x => x != "").ToList();
                                            //function.Knowledges = ExtractListItems(rows, j + 1).Where(x => x != "").ToList();
                                            var skillKnowledgeRow = j;

                                            while (true)
                                            {

                                                var functionNameColumnCellText = GetCellText(rows, skillKnowledgeRow, 0);

                                                if (functionNameColumnCellText.Contains("Трудовая функция"))
                                                {
                                                    functionNameColumnCellText = Regex.Replace(functionNameColumnCellText, @"Трудовая функция \d+:", "").Trim();
                                                    functionNameColumnCellText = Regex.Replace(functionNameColumnCellText, @"Трудовая функция \d+", "").Trim();
                                                }

                                                if (functionNameColumnCellText != String.Empty && functionNameColumnCellText != function.FunctionName)
                                                {
                                                    break;
                                                }

                                                var cellText = GetCellText(rows, skillKnowledgeRow, 2);
                                                if (cellText.Contains("Умения:"))
                                                {
                                                    function.Skills.AddRange(ExtractListItems(rows, skillKnowledgeRow+1, 2,0).Where(x => x != "").ToList());
                                                }
                                                if (cellText.Contains("Знания:"))
                                                {
                                                    function.Knowledges.AddRange(ExtractListItems(rows, skillKnowledgeRow+1, 2, 0).Where(x => x != "").ToList());
                                                }
                                                skillKnowledgeRow++;

                                            }

                                            function.Skills = function.Skills.Distinct().ToList();
                                            function.Knowledges = function.Knowledges.Distinct().ToList();
                                        ...
                                        else if (rows[j].InnerText.Contains("Трудовая функция"))
                                        {
                                            break;  // Stop when the next "Трудовая функция" starts
                                        }
                                    }
                                    catch (IndexOutOfRangeException ex)
                                    {
        ...
        private string GetCellText(List<TableRow> rows, int rowIndex, int column)
        {
            ...
            if(rowIndex < rows.Count)
            {
               ...
               .Select(line =>
               {
                   var line_selected = line.Elements().Skip(line.Elements().Count() == 1 ? 0 : 1).Select(s => s.InnerText).ToList();
        ...
        private List<string> ExtractListItems(List<TableRow> rows, int rowIndex, int column = 1, int skipElements = 1)
        {
            ...
            if (rowIndex < rows.Count)
            {
                ...
                if (cells.Count > 1)
                {
                    list = cells[column].Elements<Paragraph>().Skip(skipElements).ToList()
